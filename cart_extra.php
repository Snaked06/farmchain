	...
	if(isset($_SESSION['user'])){
		...
		try{
			...
			foreach($stmt as $row){
				...
				$output .= "{\"Productname\":\"".substr($productname, 0, -1)."\",\"price\":\"".substr($price, 0, -1)."\",\"quantity\":\"".substr($quantity, 0, -1)."\"}";
		...
		if(count($_SESSION['cart']) != 0){
			...
			foreach($_SESSION['cart'] as $row){
				...
				$output .= "{\"Productname\":\"".$row['name']."\",\"price\":".number_format($row['price'], 2).",\"quantity\":".$row['quantity']."}";
